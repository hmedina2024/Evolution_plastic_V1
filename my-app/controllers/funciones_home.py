        ...
        if action == 'save_and_notify':
            ...
            if destinatarios_ids_str:
                try:
                    ...
                    app.logger.error(f"Error procesando destinatarios manuales en registro OP: {e}")
            ...
            try:
                ...
                app.logger.error(f"Error consultando correos fijos en registro OP: {e}")
...
def procesar_actualizar_form_op(codigo_op, dataForm, files):
    ...
    id_cliente_val = None
    if id_cliente_str and id_cliente_str.strip():
        try:
            val = int(id_cliente_str)
            if val != orden.id_cliente:   # ← clave
                if not Clientes.query.filter_by(id_cliente=val, fecha_borrado=None).first():
                    errores.append(f"Cliente con ID '{val}' no encontrado.")
            id_cliente_val = val
        except ValueError:
            errores.append("ID Cliente debe ser un número entero válido.")
