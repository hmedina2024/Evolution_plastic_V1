...
@app.route('/generar-pdf-op/<codigo_op>')
def generar_pdf_op(codigo_op):
    ...
    data_logistica = [
        ['Empaque', 'Logística', 'Instructivo', 'Estado Proyecto'],
    ...
    if detalle_op.get('renders'):
        ...
        for i, render_file in enumerate(detalle_op['renders']):
            ...
            if os.path.exists(r_path):
                ...
                img_obj = Image(r_path, width=2*inch, height=2*inch, kind='proportional')
                row.append(img_obj)
    ...
    if detalle_op.get('piezas'):
        ...
        for idx, pieza in enumerate(detalle_op['piezas'], 1):
            ...
            # Tabla Info Pieza
            data_pieza = [
                ['Cant', 'Montaje', 'Tamaño Mont.', 'Material', 'Prov. Ext.'],
                [t(pieza['cantidad']), t(pieza['montaje']), t(pieza['montaje_tamano']), p_cell(pieza['material']), t(pieza.get('proveedor_externo'))],
                ['Ancho', 'Alto', 'Fondo', 'Cant. Mat.', 'Tipo Molde'],
                [t(pieza['ancho_pieza']), t(pieza['alto_pieza']), t(pieza['fondo_pieza']), t(pieza['cantidad_material']), t(pieza['tipo_molde'])]
            ]
            ...
            if pieza.get('especificaciones'):
                ...
                for esp in pieza['especificaciones']:
                    data_specs.append([
                        p_cell(esp.get('item'), ParagraphStyle('s', fontSize=8)),
                        t(esp.get('calibre')), t(esp.get('largo')), t(esp.get('ancho')),
                        t(esp.get('unidad')), t(esp.get('cantidad_especificacion')),
                        t(esp.get('kg')), t(esp.get('retal_kg')), t(esp.get('reproceso'))
                    ])
